...
subtitle: Cost of Living and DEI Report
...
Rent, Utilities, Food, Transportation, Health out-of-pocket, household goods
...
ggplot(col_data)+
  ...
  labs(x = 'Monthly Housing Expenses', y= '',
       title = 'Average Monthly Housing Cost') +
...
housing_cost_plot <- ggplot(living_sit_sum)+
  ...
  geom_errorbar(aes(x = living_situation, ymin = mean_house,
                ...
                width = 0.25) +
  labs(title = 'Housing Cost by Living Situation',
       x = '', y = 'Monthly Housing Cost [$]')+
  scale_y_continuous(expand = c(0,0))+
...
The Average students spends \$`r round(mean(col_data$total_col),2)` annually. The majority of students (75% high density interval) spend between \$`r hdi(col_data$total_col, credMass = .75)[1]`/year and \$`r hdi(col_data$total_col, credMass = .75)[2]`.

Notably, this is lover than the living wage for a single individual in Columbia, SC
...
So when we adjust the cost of living based on whether or not people are receiving financial support, we can see some different trends. 
...
col_stipends <- stipends_clean |> 
  select(RespondentId, stipend_amt) |> 
  right_join(col_data, by = 'RespondentId')


col_stipends$take_loans <- col_stipends$take_loans |> 
  sapply(function(x) switch(x, 'Yes' = 1, 'No' = 0))
